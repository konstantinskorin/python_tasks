...
class Matrix(object):
    ...
    def is_identity(self):
        ...
        for m in range(len(self.rows)):
            for m in range(len(self.rows[n])):
                if n == m:
                    if self.rows[n][m] != 1:
                        res = False
                else:
    ...
    def makeRandom(cls, m, n, low=0, high=10):
        ...
        return obj
        print(obj)
        print("Единичная - " + str(obj.is_identity()))
        print("Квадратная - " + str(obj.is_square()))
        print("Нулевая - " + str(odj.is_zero()))
        print("Диагональная - " + str(obj.is_diagonal()) + "\n")
